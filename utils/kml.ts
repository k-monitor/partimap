...
const EXPORTED_EXTRA_STROKE_NAME = 'partimapExtraStroke';
...
function prepareKmlForExport(kmlString: string) {
	...
	kml.querySelectorAll('Placemark').forEach((p) => {
		...
		renameData(ed, 'extraStroke', EXPORTED_EXTRA_STROKE_NAME);
...
function prepareKmlForImport(kmlString: string) {
	...
	placemarks.forEach((p, i) => {
		...
		renameData(ed, EXPORTED_EXTRA_STROKE_NAME, 'extraStroke');
